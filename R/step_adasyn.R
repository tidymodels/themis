...
#'   step_adasyn(Home, over_ratio = 0.47461) %>%
#'   prep()
#'
#' training <- up_rec %>%
#'   bake(new_data = NULL) %>%
...
prep.step_adasyn <- function(x, training, info = NULL, ...) {
  ...
  if (length(col_name) == 1) {
    check_column_factor(training, col_name, call = expr(step_adasyn()))
  }

  predictors <- setdiff(info$variable[info$role == "predictor"], col_name)
  check_type(training[, predictors], TRUE, call = expr(step_adasyn()))
  check_na(
    select(training, all_of(c(col_name, predictors))),
