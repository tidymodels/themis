...
#'   step_smote(Home, over_ratio = 0.47461) %>%
#'   prep()
#'
#' training <- up_rec %>%
#'   bake(new_data = NULL) %>%
...
prep.step_smote <- function(x, training, info = NULL, ...) {
  ...
  check_at_most_one(col_name, expr(step_smote()))

  if (length(col_name) == 1) {
    check_column_factor(training, col_name, call = expr(step_smote()))
  }

  predictors <- setdiff(info$variable[info$role == "predictor"], col_name)
  check_type(training[, predictors], TRUE, call = expr(step_smote()))
  check_na(
