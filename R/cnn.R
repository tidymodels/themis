...
#'
#' @seealso [cnn()] for direct implementation
...
#' @export
prep.step_cnn <- function(x, training, info = NULL, ...) {
...
#' @export
bake.step_cnn <- function(object, new_data, ...) {
  ...
  # cnn with seed for reproducibility
  with_seed(
    seed = object$seed,
    code = {
      cnn_data <- cnn_impl(
  ...
  if (length(cnn_data) > 0) {
    new_data <- new_data[-cnn_data, ]
  }
...
#' @export
required_pkgs.step_cnn <- function(x, ...) {
  c("themis")
}
