...
#' 179-186).
#'
#' @seealso [oss()] for direct implementation
...
#' @export
prep.step_oss <- function(x, training, info = NULL, ...) {
...
#' @export
bake.step_oss <- function(object, new_data, ...) {
  ...
  # oss with seed for reproducibility
  with_seed(
    seed = object$seed,
    code = {
      oss_data <- oss_impl(
  ...
  if (length(oss_data) > 0) {
    new_data <- new_data[-oss_data, ]
  }
...
#' @export
required_pkgs.step_oss <- function(x, ...) {
  c("themis")
}
