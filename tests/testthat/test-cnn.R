...
test_that("distance_with allows non-numeric columns to be present", {
  ...
  expect_no_error(
    recipe(class ~ ., data = df_mixed) |>
      step_cnn(class, distance_with = c(x, y)) |>
      prep() |>
      bake(new_data = NULL)
  )
})
...
test_that("distance_with errors on non-numeric column", {
  ...
  expect_snapshot(
    error = TRUE,
    recipe(class ~ ., data = df_mixed) |>
      step_cnn(class, distance_with = c(x, name)) |>
      prep()
  )
})
...
test_that("distance argument accepted by step_cnn()", {
  expect_no_error(
    recipe(class ~ x + y, data = circle_example) |>
      step_cnn(class, distance = "euclidean") |>
      prep() |>
      bake(new_data = NULL)
  )
  expect_no_error(
    recipe(class ~ x + y, data = circle_example) |>
      step_cnn(class, distance = "cosine") |>
      prep() |>
      bake(new_data = NULL)
  )
  expect_no_error(
    recipe(class ~ x + y, data = circle_example) |>
      step_cnn(class, distance = "mahalanobis") |>
      prep() |>
      bake(new_data = NULL)
  )
  expect_no_error(
    recipe(class ~ x + y, data = circle_example) |>
      step_cnn(class, distance = "manhattan") |>
      prep() |>
      bake(new_data = NULL)
  )
  expect_no_error(
    recipe(class ~ x + y, data = circle_example) |>
      step_cnn(class, distance = "chebyshev") |>
      prep() |>
      bake(new_data = NULL)
  )
})

test_that("bad distance arg for step_cnn()", {
  expect_snapshot(
    error = TRUE,
    bake(
      prep(step_cnn(
