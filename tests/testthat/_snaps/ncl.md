# errors when not enough observations for neighbors
    ...
    Code
      bake(prep(step_ncl(recipe(class ~ x + y, data = small), class, neighbors = 5,
...
# errors if character are present
    ...
    Code
      prep(step_ncl(recipe(~., data = df_char), x))
    Condition
      Error in `step_ncl()`:
      Caused by error in `prep()`:
...
# NA in response
    ...
    Code
      prep(step_ncl(recipe(Status ~ Age, data = credit_data0), Status))
    Condition
      Error in `step_ncl()`:
      Caused by error in `prep()`:
...
# distance_with errors on non-numeric column
    ...
    Code
      prep(step_ncl(recipe(class ~ ., data = df_mixed), class, distance_with = c(x,
        name)))
    Condition
      Error in `step_ncl()`:
      Caused by error in `prep()`:
...
# bad distance arg for step_ncl()

    Code
      bake(prep(step_ncl(recipe(class ~ x + y, data = circle_example), class,
...
# bad args
    ...
    Code
      step_ncl(recipe(~., data = mtcars), seed = TRUE)
    Condition
      Error in `step_ncl()`:
...
---
    ...
    Code
      prep(step_ncl(recipe(class ~ x + y, data = circle_example), class, neighbors = -
      1))
    Condition
      Error in `step_ncl()`:
      Caused by error in `prep()`:
...
---
    ...
    Code
      prep(step_ncl(recipe(class ~ x + y, data = circle_example), class,
      threshold_clean = -1))
    Condition
      Error in `step_ncl()`:
      Caused by error in `prep()`:
