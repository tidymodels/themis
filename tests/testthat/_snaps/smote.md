# errors if there isn't enough data
    ...
    Condition
      ...
      ! The minority class "dummy" does not have enough observations to perform SMOTE.
...
# indicator_column bad args
    ...
    Code
      step_smote(recipe(class ~ x + y, data = circle_example), class,
      indicator_column = 1)
    Condition
      Error in `step_smote()`:
...
---
    ...
    Code
      prep(step_smote(recipe(class ~ x + y, data = circle_example), class,
      indicator_column = ""))
    Condition
      Error in `step_smote()`:
