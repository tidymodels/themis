...
test_that("indicator_column adds logical column marking synthetic rows", {
  rec <- recipe(class ~ x + y, data = circle_example) |>
    step_adasyn(class, indicator_column = ".new_row") |>
    prep()
...
test_that("indicator_column bad args", {
  expect_snapshot(
    error = TRUE,
    recipe(class ~ x + y, data = circle_example) |>
      step_adasyn(class, indicator_column = 1)
  )
  expect_snapshot(
    error = TRUE,
    recipe(class ~ x + y, data = circle_example) |>
      step_adasyn(class, indicator_column = "") |>
      prep()
  )
  expect_snapshot(
    error = TRUE,
    recipe(class ~ x + y, data = circle_example) |>
      step_adasyn(class, indicator_column = "x") |>
      prep()
  )
})
