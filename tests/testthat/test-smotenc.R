...
test_that("indicator_column adds logical column marking synthetic rows", {
  rec <- recipe(class ~ x + y, data = circle_example) |>
    step_smotenc(class, indicator_column = ".new_row") |>
    prep()
...
test_that("indicator_column bad args", {
  expect_snapshot(
    error = TRUE,
    recipe(class ~ x + y, data = circle_example) |>
      step_smotenc(class, indicator_column = 1)
  )
  expect_snapshot(
    error = TRUE,
    recipe(class ~ x + y, data = circle_example) |>
      step_smotenc(class, indicator_column = "") |>
      prep()
  )
  expect_snapshot(
    error = TRUE,
    recipe(class ~ x + y, data = circle_example) |>
      step_smotenc(class, indicator_column = "x") |>
      prep()
  )
})
...
test_that("step_smotenc() samples each class to its own target (#323)", {
  set.seed(1)
  df <- data.frame(
    x = rnorm(140),
    y = factor(sample(LETTERS[1:3], 140, replace = TRUE)),
  ...
  res <- recipe(class ~ ., data = df) |>
    step_smotenc(class, over_ratio = c(a = 1, b = 0.75)) |>
    prep() |>
    bake(new_data = NULL)
...
test_that("step_smotenc() leaves a class alone when its target is on the wrong side (#323)", {
  set.seed(1)
  df <- data.frame(
    x = rnorm(140),
    y = factor(sample(LETTERS[1:3], 140, replace = TRUE)),
  ...
  res <- recipe(class ~ ., data = df) |>
    step_smotenc(class, over_ratio = c(c = 0.5)) |>
    prep() |>
    bake(new_data = NULL)
...
test_that("step_smotenc() checks `over_ratio` names when prepped (#323)", {
  set.seed(1)
  df <- data.frame(
    x = rnorm(140),
    y = factor(sample(LETTERS[1:3], 140, replace = TRUE)),
  ...
  expect_snapshot(
    error = TRUE,
    recipe(class ~ ., data = df) |>
      step_smotenc(class, over_ratio = c(a = 1, potato = 1)) |>
      prep()
  )
})
